#include "silly-string.h"

#define SUBTREES_LEN 128

typedef struct silly__nodeT silly_node;
struct silly__nodeT {
    const void* value;
    struct silly__nodeT** subtrees;
};

void silly_free(silly_string* root) {
    if (!root) {
        return;
    }
    for (int i = 0; i < SUBTREES_LEN; i++) {
        silly_free(root->subtrees[i]);
    }
    free(root->subtrees);
    free(root);
}

silly_node* silly_init(void) {
    silly_node* node = (silly_node*)malloc(sizeof(silly_node));
    node->value = NULL;
    node->subtrees = (silly_node**)calloc(SUBTREES_LEN, sizeof(silly_node*));
    return node;
}

const void* silly_get(silly_string* root, const char* key) {
    silly_string* current = root;
    char c;
    while ((c = *(key++))) {
        current = current->subtrees[(int)c];
        if (current == NULL) {
            return NULL;
        }
    }
    return current->value;
}

const void* silly_get_ci(silly_string* root, const char* key) {
    silly_string* current = root;
    char c;
    while ((c = *(key++))) {
        if (c >= 'a' && c <= 'z') {
            c -= 32;
        }
        current = current->subtrees[(int)c];
        if (current == NULL) {
            return NULL;
        }
    }
    return current->value;
}

void silly_insert(silly_string* root, const char* key, const void* value) {
    silly_string* current = root;
    char c;
    while ((c = *(key++))) {
        if (!current->subtrees[(int)c]) {
            current->subtrees[(int)c] = silly_init();
        }
        current = current->subtrees[(int)c];
    }
    current->value = value;
}

void silly_insert_ci(silly_string* root, const char* key, const void* value) {
    silly_string* current = root;
    char c;
    while ((c = *(key++))) {
        if (c >= 'a' && c <= 'z') {
            c -= 32;
        }
        if (!current->subtrees[(int)c]) {
            current->subtrees[(int)c] = silly_init();
        }
        current = current->subtrees[(int)c];
    }
    current->value = value;
}

const void* silly_get_ci_from_case_sensitive(silly_string* root, const char* key) {
    char c = *(key++);
    if (!c) {
        return root->value;
    }
    silly_string* current = root->subtrees[(int)c];
    if (!current) {
        if (c >= 'A' && c <= 'Z') {
            current = root->subtrees[c + 32];
            if (!current) {
                return NULL;
            } else {
                return silly_get_ci_from_case_sensitive(current, key);
            }
        } else if (c >= 'a' && c <= 'z') {
            current = root->subtrees[c - 32];
            if (!current) {
                return NULL;
            } else {
                return silly_get_ci_from_case_sensitive(current, key);
            }
        } else {
            return NULL;
        }
    }
    const void* value = silly_get_ci_from_case_sensitive(current, key); 
    if (value) {
        return value;
    } else if (c >= 'A' && c <= 'Z') {
        current = root->subtrees[c + 32];
        if (!current) {
            return NULL;
        } else {
            return silly_get_ci_from_case_sensitive(current, key);
        }
    } else if (c >= 'a' && c <= 'z') {
        current = root->subtrees[c - 32];
        if (!current) {
            return NULL;
        } else {
            return silly_get_ci_from_case_sensitive(current, key);
        }
    } else {
        return NULL;
    }
}

